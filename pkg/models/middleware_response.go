...
func ResponseInterceptor(defaultLangs string, availableLanguages []string) grpc.UnaryServerInterceptor {
		...
		if err != nil {
			...
			if st, ok := status.FromError(err); ok {
				if md, ok := metadata.FromIncomingContext(ctx); ok {
					lang := utils.GetAcceptedLanguageFromGrpcCtx(ctx, md, availableLanguages, defaultLangs)
