...
func ReadTestFile(dirName string, fileName string) ([]byte, error) {
	testsDir, err := FindDir(dirName)
	if err != nil {
		return nil, err
	}

	return os.ReadFile(filepath.Join(testsDir, fileName))
}

func ReadTestImage(dirName string, fileName string) (string, error) {
	data, err := ReadTestFile(dirName, fileName)
	if err != nil {
		return "", err
	}
